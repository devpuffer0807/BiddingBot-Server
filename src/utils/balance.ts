...
const balanceLocks = new Map<string, boolean>();
const lockQueue = new Set<string>();
const MAX_QUEUE_SIZE = 64;

async function acquireLock(address: string, balanceType: 'weth' | 'beth'): Promise<boolean> {
  const lockKey = `${balanceType}_${address}`;
  if (balanceLocks.get(lockKey)) {
    if (lockQueue.size >= MAX_QUEUE_SIZE) {
      return false;
    }
    lockQueue.add(lockKey);
    return false;
  }
  balanceLocks.set(lockKey, true);
  lockQueue.delete(lockKey);
  return true;
}

function releaseLock(address: string, balanceType: 'weth' | 'beth'): void {
  const lockKey = `${balanceType}_${address}`;
  balanceLocks.delete(lockKey);
}
